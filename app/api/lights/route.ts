...
// Easter egg: clicking the site name five times sends this animation
// to the public LED wall at blinken.org.
export async function GET() {
  const resp = await fetch("https://blinken.org/api/0/publish", {
    method: "POST",
  ...
  return NextResponse.json({ status: await resp.text() });
}
